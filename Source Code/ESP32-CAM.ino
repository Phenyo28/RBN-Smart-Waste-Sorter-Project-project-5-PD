...
uint8_t broadcastAddress[] = {0x6C, 0xC8, 0x40, 0x8C, 0x34, 0xD4};
...
void loop()
{
   ...
   if (result.classification[i].value > top_value) {
            ...
            top_label = ei_classifier_inferencing_categories[i];
        }
   // Send top label via UART
    Serial2.print("Detected: ");
    Serial2.print(top_label);
    Serial2.print(" (");
    Serial2.print(top_value, 3);
    Serial2.println(")");

    ei_printf("Top result sent over UART: %s (%.3f)\n", top_label, top_value);
#endif
